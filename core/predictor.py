...
class Predictor:
    ...
    def predict(self, inputs_cat: torch.Tensor, inputs_num: torch.Tensor, return_probs: bool = False) -> torch.Tensor:
        ...
        Args:
            inputs_cat (torch.Tensor): Input tensor (e.g., image or batch of images)
            inputs_num (torch.Tensor): Input tensor (e.g., image or batch of images)
        ...
        with torch.no_grad():
            ...
            inputs_cat = inputs_cat.to(self.device, non_blocking=True)
            inputs_num = inputs_num.to(self.device, non_blocking=True)
            
            # Make predictions using model
            outputs = self.model(inputs_cat, inputs_num)
